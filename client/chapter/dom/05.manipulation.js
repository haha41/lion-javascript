/* -------------------------- */
/* DOM Manipulation           */
/* -------------------------- */


/* 노드 생성 메서드 --------------------------------------------------------- */

// - document.createElement(tagName) – 태그 이름을 사용해 새로운 요소 생성
// - document.createTextNode(value) – 새로운 텍스트 노드 생성
// - element.cloneNode(deep) – 요소 복제. deep==true일 경우 모든 자손 요소도 복제

let div = document.createElement('div');
div.className = 'box';
div.textContent = '상자';



/* 노드 삽입, 삭제 메서드 ---------------------------------------------------- */

// - node.append(노드나 문자열) – node 끝에 노드를 삽입
// - node.prepend(노드나 문자열) – node 맨 앞에 노드를 삽입
// - node.before(노드나 문자열) – node 이전에 노드를 삽입
// - node.after(노드나 문자열) – node 다음에 노드를 삽입
// - node.replaceWith(노드나 문자열) – node를 대체
// - node.remove() – node를 제거


/* '오래된' 메서드 ----------------------------------------------------------- */

// - parent.appendChild(node)
// - parent.insertBefore(node, nextSibling)
// - parent.removeChild(node)
// - parent.replaceChild(newElement, node)


/* 특정 위치에 삽입 --------------------------------------------------------- */

// - elementNode.insertAdjacentHTML(어디에?, 값);
// - insertAdjacentElement
// - insertAdjacentText

// - "beforebegin" – elem 바로 앞에 html을 삽입
// - "afterbegin" – elem의 첫 번째 자식 요소 바로 앞에 html을 삽입
// - "beforeend" – elem의 마지막 자식 요소 바로 다음에 html을 삽입
// - "afterend" – elem 바로 다음에 html을 삽입

// const body = document.body;
// body.insertAdjacentHTML('beforeend','<div class="box">상자</div>')

const h1 = getNode('h1');

// const template = '<div class="box">상자</div>'
// const template = `<div class="box">${ 10 + 30 }</div>` // 변수 또는 식도 가능함
const template = /* html */ ` 
    <div class="box">${ 10 + 30}</div>
`

h1.insertAdjacentHTML('beforebegin', template)



const data = ['빨래하기', '게임하기', '유튜브 보기', '산책하기'];

// forEach => 반환 x
// reduce => 아무거나 반환
// filter => 배열을 반환, 예) const data = ['빨래하기', '개랑 게임하기', '유튜브 보기', '개랑 산책하기']; 에서 개랑 하는 것만 뽑아낼 때
// map => 배열을 반환
// for문

const todo = getNode('.todo');

// 1. 태그를 생성하기
// 2. 태그 안에 배열 값 넣기
`<li>${data[0]}</li>`

/* function render(){

  return(
   
    <ul>
      {
       data.map(item => `<li>${item}</li>`)
      }
    </ul>
  )
}
 */

// 3. 생성된 태그를 내보내기 (배열)
// data.map((item)=>{
//   console.log(item); // 배열 까집고 그 안의 아이템 반복 (빨래하기, 게임하기, 유튜브 보기, 산책하기)
// })

const todoList = data.map((item)=>{
  return `<li>${item}</li>` // 빨래하기, 게임하기, 유튜브 보기, 산책하기, 값을 반환해야 하기 때문에 return 필요
})

// todo.insertAdjacentHTML('beforeend',todoList) // 배열의 콤마까지 순환되서 나옴. 왜나하면 2번째 인자가 text여서

// for(let i = 0; i < todoList.length; i++){
//   todo.insertAdjacentHTML('beforeend',todoList[i])
// }

console.log(todoList); // (4) ['<li>빨래하기</li>', '<li>게임하기</li>', '<li>유튜브 보기</li>', '<li>산책하기</li>']


// 4. 내보낸 배열 순환하기
// 값을 반환할 필요는 없으므로 forEach

// todoList.forEach((item)=>{
//   console.log(item);
// })

todoList.forEach((item)=>{
  // 5. 반복문 안에서 렌더링하기
  // todo.insertAdjacentHTML('beforeend',item)
  insertLast(todo, item) // beforeend => insertLast
})



// ElementNode.insertAdjacentHTML('beforebegin', 'text')
// ElementNode.insertAdjacentHTML('afterbegin', 'text')
// ElementNode.insertAdjacentHTML('beforeend', 'text')
// ElementNode.insertAdjacentHTML('afterend', 'text')

insertLast('.todo', '<li>문자하기</li>')

// insertBefore('.todo',text)

// insertLast('.todo',text)

// 6. 렌더링
