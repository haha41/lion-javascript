...
// 바로 실행 되는게 아닌, 사용자가 어떤 인터렉션(행동)을 했을 때 코드 로직이 실행되는
...
try {
  ...
  console.log(e.stack); // ReferenceError: b is not defined at 21.tryCatch.js:21:7
  // let a = 5;
  // let b = 10;
  // a + b;
}
...
try {
  // JSON.parse(data) // pasre : 데이터를 제이슨에 넣게 되면 원래의 자료값으로 되돌려 놓음 (문자 -> 객체)
  const user = JSON.parse(data); // 서버로부터 넘어온 값을 객체로 다시 변환.
  ...
  // console.log(e.message); // 해당 키 값은 존재하지 않습니다.
...
// 에러 유무와 상관 없이 실행됨
  ...
  console.log('에러가 발생하여도 해당 코드는 작동됩니다.');
}